...
Component({
  ...
  methods: {
    ...
    init() {
      ...
      const time = this.data.propA//this.formatTime(dateTime, 'hh:mm')
      const selected = [year, month, dayInMonth]
      this.setData({ currYear: year, currMonth: month, dayInWeek, dayInMonth, week, time, selected, showModal: this.data.showModal})
      const emptyGrids = this.calEmptyGrid(year, month)
      const days = this.calDays(year, month)
      this.setData({ emptyGrids, days })
    },
